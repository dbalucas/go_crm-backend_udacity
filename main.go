...
func initDB(db gorm.DB) {
	...
	// run auto migrato for the database structure always
	db.AutoMigrate(&Customer{})

	// fill initial test data if not exists
	if result := db.Raw("SELECT * FROM customers").Scan(result); result.RowsAffected == nil {
	...
	result := db.CreateInBatches(&customerRepository.customers, 15)
	if result.RowsAffected != 0 {
		fmt.Printf("database initialized with %v entities. \n", result.RowsAffected)
	} else {
		fmt.Printf("error during database init: %v \n", result.Error)
	}
}
